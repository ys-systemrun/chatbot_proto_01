# database 構成の state（ADR-0039）。ネットワーク基盤 + RDS(pgvector) + シード用 ECS 周辺を1つの
# Terraform root に統合したもの（旧 verify-network + verify-database + verify の db_init 部分）。
#
...
#     -backend-config="key=database/state.tfstate" `
