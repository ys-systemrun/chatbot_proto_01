...
setlocal
rem chatbot_invitro AWS deploy (full pipeline). ADR-0040: runs in a Python container.
rem database -> app(gate closed) -> seed -> app(gate open). Confirms once before creating billable resources.
rem Prereq: Docker Desktop running; terraform\.env filled (AWS creds + all TF_VAR_*).
call "%~dp0deploy\run.bat" apply-all
echo.
